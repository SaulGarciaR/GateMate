...
set TOP=aes256_uart
set VLOG_SRC=src/aes256_uart.v src/aes256_enc.v src/byte_mix.v src/counters.v src/key_exp.v src/mix_columns.v src/mix_w.v src/MUL3.v src/round_constant.v src/round_key.v src/sbox.v src/shift_rows.v src/sub_bytes.v src/xtimes.v
set LOG=0
...
if "%1"=="synth_vlog" (
  if %LOG%==1 (
	...
	:: Synthesize Verilog and VHDL modules
if "%1"=="synth_serial" (
  if %LOG%==1 (
    start /WAIT /B %YOSYS% -l log/synth.log -p "read -sv %VLOG_SRC%; ghdl --warn-no-binding -C --ieee=synopsys src/serial_tx.vhd -e serial_tx; ghdl --warn-no-binding -C --ieee=synopsys src/serial_rx.vhd -e serial_rx; synth_gatemate -top %TOP% -nomx8 -vlog net/%TOP%_synth.v"
  ) else (
    start /WAIT /B %YOSYS% -ql log/synth.log -p "read -sv %VLOG_SRC%; ghdl --warn-no-binding -C --ieee=synopsys src/serial_tx.vhd -e serial_tx; ghdl --warn-no-binding -C --ieee=synopsys src/serial_rx.vhd -e serial_rx; synth_gatemate -top %TOP% -nomx8 -vlog net/%TOP%_synth.v"
  )
)
